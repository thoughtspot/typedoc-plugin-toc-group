...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "typedoc/dist/lib/models/reflections", "typedoc/dist/lib/converter/components", "typedoc/dist/lib/output/events", "typedoc/dist/lib/output/models/NavigationItem", "typedoc/dist/lib/converter/converter", "typedoc/dist/lib/output/plugins/TocPlugin"], factory);
    }
        ...
        onBeginResolve(context) {
            ...
            for (const key in reflections) {
                ...
                for (const tag of comment.tags) {
                    ...
                    if (DEPRECATED_REGEXP.test(`@${tag.tagName}`))
                        deprecatedData.add(ref.name);
                    // add special tags
                    if (this.regexp.test(`@${tag.tagName}`)) {
                        groupedData.push(ref.name);
                        const groupKey = tag.text.split(/\r\n?|\n/)[0];
                        if (!mapedTocData[groupKey])
                            mapedTocData[groupKey] = [];
            ...
            if (typeof mapedTocData === 'object' && Object.keys(mapedTocData).length) {
                ...
                if (!mapedTocData[DEFAULT_UNGROUPED_NAME]) {
                    ...
                    if (defaultGroups.length)
                        mapedTocData[DEFAULT_UNGROUPED_NAME] = defaultGroups;
                }
                const updatedToc = Object.keys(mapedTocData).map((key) => {
                    ...
                    root.children = page.toc.children.filter((item) => {
                        if (regexp.test(`@!${item.reflection.kind}`))
                            return false;
